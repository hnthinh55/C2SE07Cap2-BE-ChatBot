...
with open('./data/trong_trot.json', 'r') as f4:
    data4 = json.load(f4)
with open('./data/category.json', 'r') as f8:
    data8 = json.load(f8)
with open('./data/basic_comunication.json', 'r', encoding='utf-8') as f5:
    data5 = json.load(f5)
with open('./data/weather.json', 'r') as f6:
    data6 = json.load(f6)
with open('./data/product.json', 'r') as f7:
    data7 = json.load(f7)


# Kết hợp dữ liệu từ hai file JSON
merged_data = {"intents": data1["intents"] + data2["intents"] +data3["intents"] + data8["intents"] +data5["intents"]+ data6["intents"]+ data7["intents"]+ data4["intents"]}
